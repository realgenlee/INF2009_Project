...
# FIX: Reduced from 3 to 2 — fewer votes needed to flip contact status,
# reducing stabilisation lag significantly.
CONTACT_VOTE_N = 1
...
# --- Severity debounce
# FIX: Asymmetric debounce — upgrading to ALERT is slow (conservative),
# downgrading to WARN/NORMAL is fast (responsive).
SEVERITY_UPGRADE_S   = 0.5   # WARN -> ALERT: slow, avoid false alarms
SEVERITY_DOWNGRADE_S = 0.3   # ALERT/WARN -> lower: fast, responsive

# --- Stage 2 hard gate — IR must be in valid perfusion range
IR_VALID_MIN = 50_000
IR_VALID_MAX = 200_000

# --- AC/DC collapse threshold — instant poor-contact detection
ACDC_MIN_VALID = 0.001

# --- Stage 2 warm-up — don't run anomaly detection until contact stable
...
def compute_spo2_est(red_window, ir_window) -> Optional[float]:
    red = np.asarray(list(red_window), dtype=np.float32)
    ir  = np.asarray(list(ir_window), dtype=np.float32)
...
def build_contact_feature_vector(
    ir_window,
    red_window,
    t_room_window,
    hr_window,
    ...
    """Build feature vector for Stage-1 contact classifier using the same
    ...
    feat["acdc_over_p2p_mean"]  = float(np.mean(acdc_arr / (p2p_arr + 1e-6))) if acdc_arr.size > 0 else 0.0
    feat["p2p_over_ir_mean"]    = float(np.mean(p2p_arr / (np.abs(ir_arr) + 1e-6))) if ir_arr.size > 0 else 0.0

    feat["red_ir_ratio_mean"]   = float(np.mean(red_arr / (ir_arr + 1e-6))) if red_arr.size > 0 and ir_arr.size > 0 else 0.0
    feat["red_ir_ratio_std"]    = float(np.std(red_arr / (ir_arr + 1e-6))) if red_arr.size > 0 and ir_arr.size > 0 else 0.0
    ...
    feat["hr_over_ir_mean"]       = float(np.mean(hr_arr / (np.abs(ir_arr) + 1e-6))) if hr_arr.size > 0 and ir_arr.size > 0 else 0.0
...
# ==============================================================================
# Severity debounce
# FIX: Asymmetric timing — upgrading severity (WARN->ALERT) is slow to avoid
# false alarms; downgrading (ALERT->WARN->NORMAL) is fast for responsiveness.
# ==============================================================================
class SeverityDebounce:
    """
    Prevents momentary spikes from flipping the LED.
    - Upgrades (higher severity) only after SEVERITY_UPGRADE_S of persistence.
    - Downgrades (lower severity) only after SEVERITY_DOWNGRADE_S.
    """
...
# Moves the blocking dht.temperature call off the 20ms inference loop entirely.
# The main loop reads _dht_value non-blocking via a lock — near-zero cost.
# ==============================================================================
_dht_lock  = threading.Lock()
_dht_value = None   # shared, written by thread, read by main loop
...
_prof_t_s1_ms    = 0.0
_prof_t_s2_ms    = 0.0
PROF_REPORT_EVERY = 100   # print timing summary every 100 cycles
...
try:
    with SMBus(1) as bus:
        ...
        # Stage-1 vote buffer (FIX: reduced to 2)
        contact_vote_buf    = deque(maxlen=CONTACT_VOTE_N)

        # Safe defaults before first inference
        ...
        # FIX: Stage 2 warm-up timer — only run anomaly detection after
        # contact has been stable for CONTACT_WARMUP_S seconds, preventing
        # false red alerts during the initial stabilisation window.
        ...
        while True:
            ...
            try:
                ...
                if 50000 < ir < 200000:
                    ...
                    if signal_present:
                        ...
                        contact_conf       = 0.0
                        contact_label      = "unknown"
                        contact_good_since = None   # FIX: reset warm-up on signal loss
                        signal_present     = False
            ...
            # Derive PPG proxies
            current_ir_window  = list(ir_buf)
            current_red_window = list(red_buf)

            if current_ir_window:
                ir_arr_w      = np.asarray(current_ir_window, dtype=np.float32)
            ...
            spo2_est = compute_spo2_est(current_red_window, current_ir_window)
            ...
            # FIX: Reset each cycle — prevents stale severity carrying forward
            # when inference doesn't run (buffer not yet full).
            anomaly_score_this_cycle = None
            score_threshold_used     = anomaly_score_threshold
            temp_ctx_label           = temp_context.context_label()
            is_anomaly               = False
            raw_severity             = "WARN"   # safe default

            if ai_available:
                # FIX: Instant poor-contact detection via AC/DC collapse.
                # If the pulsatile component disappears, we know immediately —
                # no need to wait for the vote buffer to catch up.
                if ac_dc_ratio_v < ACDC_MIN_VALID and contact_status == "GOOD":
                    ...
                    is_anomaly         = False
                    if ai_should_print:
                        print("[AI-S1] AC/DC collapsed — instant poor-contact detection.")
                ...
                if run_full or run_short:
                    if run_short:
                        ir_win   = list(ir_buf)[-PPG_SHORT_WINDOW:]
                        red_win  = list(red_buf)[-PPG_SHORT_WINDOW:]
                        hr_win   = list(hr_buf)[-PPG_SHORT_WINDOW:]
                        spo2_win = list(spo2_buf)[-PPG_SHORT_WINDOW:]
                        acdc_win = list(acdc_buf)[-PPG_SHORT_WINDOW:]
                        p2p_win  = list(p2p_buf)[-PPG_SHORT_WINDOW:]
                    else:
                        ir_win   = list(ir_buf)[-contact_window_size:]
                        red_win  = list(red_buf)[-contact_window_size:]
                        hr_win   = list(hr_buf)[-contact_window_size:]
                        spo2_win = list(spo2_buf)[-contact_window_size:]
                        acdc_win = list(acdc_buf)[-contact_window_size:]
                        p2p_win  = list(p2p_buf)[-contact_window_size:]

                    tr_win = list(t_room_buf)

                    _t_feat0 = time.perf_counter()                      # PASO profiling
                    x_c = build_contact_feature_vector(
                        ir_window=ir_win,
                        red_window=red_win,
                    ...
                    # FIX: Lowered short-window penalty from +0.10 to +0.02
                    # so fast-recovery votes actually enter the buffer.
                    ...
                    # FIX: Recency-weighted vote — recent predictions count more
                    # than old ones, so one stale BAD vote can't hold back a
                    # clearly-good current signal.
                    if contact_vote_buf:
                    ...
                    if contact_status == "BAD" and prev_contact_status == "GOOD":
                        # Motion detected — flush stale PPG data
                        ...
                        contact_good_since = None   # FIX: reset warm-up on contact loss

                    prev_contact_status = contact_status

                    win_label = "short" if run_short else "full"
                    if ai_should_print:
                        print(f"[AI-S1] Contact: {contact_label} (conf={contact_conf:.2f}, vote={contact_status}, win={win_label})")
                    _prof_t_s1_ms += (time.perf_counter() - _t_s1_0) * 1000    # PASO profiling

                # --- Stage 2 gate checks ---

                # FIX: Warm-up timer — track how long contact has been GOOD.
                # Stage 2 only runs after CONTACT_WARMUP_S of stable contact,
                # preventing false red alerts during initial stabilisation.
                ...
                # FIX: IR hard gate — skip Stage 2 if raw IR is outside the
                # valid perfusion range, regardless of what the vote buffer says.
                ...
                if contact_status == "GOOD" and ir_valid_for_s2 and contact_warmed_up:
                    ...
                    x_np = anomaly_imputer.transform(x_s2[anomaly_feature_cols].to_numpy(dtype=np.float32))
                    ...
                    if is_anomaly:
                        ...
                        if ai_should_print:
                            print(f"[AI-S2] DEVIATION score={anomaly_score:.6f} threshold={score_threshold_used:.6f} persisting={persist_s:.1f}s spo2={spo2_est if spo2_est is not None else 'NA'}")
                        raw_severity = "ALERT"
                    else:
                        anomaly_start = None
                        persist_s     = 0.0
                        if ai_should_print:
                            print(f"[AI-S2] Normal    score={anomaly_score:.6f} threshold={score_threshold_used:.6f} spo2={spo2_est if spo2_est is not None else 'NA'}")
                        raw_severity = "NORMAL"

                elif contact_status == "GOOD" and not ir_valid_for_s2:
                    # Vote says GOOD but raw IR is physically out of range — stay yellow
                    anomaly_start = None
                    is_anomaly    = False
                    raw_severity  = "WARN"

                elif contact_status == "GOOD" and not contact_warmed_up:
                    # Contact is good but Stage 2 still in warm-up — stay yellow
                    anomaly_start = None
                    is_anomaly    = False
                    raw_severity  = "WARN"
                    if ai_should_print:
                        remaining = CONTACT_WARMUP_S - (loop_start - contact_good_since)
                        print(f"[AI-S2] Warming up... {remaining:.1f}s remaining.")
                    ...
                    raw_severity  = "WARN"
                    if ai_should_print:
                        print("[AI-S1] No/poor contact - waiting for signal (yellow).")

                else:
                    # UNKNOWN or any unhandled state
                    is_anomaly   = False
                    raw_severity = "WARN"
